...
**Релиз v0.1.0 выпущен.** MVP, фаза B (post-MVP расширения) и A6.1 (подписанный APK) закрыты.
...
- **B3** — кастомные ярлыки с CRUD в веб-морде — ✅ закрыта (commit `25977d2`)
- **A6.1** — signed release APK (R8/ProGuard, v2+v3 подпись, smoke-тест на реальном устройстве) — ✅ закрыта (commit `ca1f4da`)
- **B4 (удалённый доступ)** — отложен как отдельная будущая фаза, план в `docs/FUTURE-REMOTE-ACCESS.md`. Текущий релиз — только локальная сеть.

Remote: `https://github.com/aitherlab-dev/remote-app-start-my-linux` (публичный). Релиз: [`v0.1.0`](https://github.com/aitherlab-dev/remote-app-start-my-linux/releases/tag/v0.1.0). Тестовый стенд: Samsung Galaxy `RFCY818A5MT` + ПК `192.168.1.248:8443`.
...
- Compose, `minSdk 26`, `targetSdk 36`, `compileSdk 36`, namespace `com.remotelauncher`
...
- **Тема**: `RemoteLauncherTheme` поддерживает и light, и dark — через `isSystemInDarkTheme()`. Обе оформлены полностью (`LightColors`/`DarkColors` в `ui/theme/Theme.kt`). Активити завёрнута в `Surface(color = MaterialTheme.colorScheme.background)` внутри `setContent {}`, чтобы системный фон из манифеста не просвечивал. Визуально проверено на реальном устройстве в тёмном режиме.
...
### Release signing (A6.1, уже настроено)

- Keystore: `/home/sasha/keys/remotelauncher.keystore` (PKCS12, RSA 4096, 100 лет validity, alias `remotelauncher`)
- **Вне репо.** Потеря = невозможность выпускать обновления (Android не примет APK, подписанный другим ключом). Бэкап — вся папка `/home/sasha/keys/` (рядом лежит `README-RemoteLauncher.txt` с паролем и fingerprint)
- Пароль читается из `~/.gradle/gradle.properties`, ключи свойств: `REMOTELAUNCHER_STORE_FILE`, `REMOTELAUNCHER_STORE_PASSWORD`, `REMOTELAUNCHER_KEY_ALIAS`, `REMOTELAUNCHER_KEY_PASSWORD`. Если файл отсутствует — `assembleRelease` упадёт на `validateSigningRelease`
- APK Signature Scheme v2 + v3 (видно в `apksigner verify --verbose`), v1/v4 выключены
- Fingerprint подписывающего ключа (SHA-256): `7B:55:60:CB:94:23:86:23:59:D0:1E:08:97:41:11:87:3C:9E:9B:89:1D:8D:6E:96:A0:D4:98:2C:88:D6:A0:91`

### Будущая фаза — удалённый доступ через платный VPS-туннель

Отложена. План и модель — в `docs/FUTURE-REMOTE-ACCESS.md`. Короткая суть: сервер держит исходящий туннель к VPS автора, телефон ходит на VPS. Работает за CGNAT, не требует UPnP/DDNS, модель — платная подписка. Изначальный B4 (UPnP + DDNS) отвергнут: ломается на CGNAT и не даёт настоящего zero-config.
...
```
REMOTE-MY-LINUX/
├── server/      # Go-сервер (готов, покрытие 82–100%)
├── packaging/   # systemd unit + install.sh/uninstall.sh
├── android/     # Kotlin-клиент (готов, релизный APK v0.1.0)
└── docs/        # ТЗ, PROGRESS, FUTURE-REMOTE-ACCESS, заметки
```
...
```bash
cd android
./gradlew :app:assembleDebug                                  # debug APK (без подписи релиза)
./gradlew :app:assembleRelease                                # signed release APK (R8 + v2+v3)
./gradlew :app:testDebugUnitTest                              # JVM unit-тесты
adb install -r app/build/outputs/apk/debug/app-debug.apk      # поставить debug
adb install -r app/build/outputs/apk/release/app-release.apk  # поставить release (подписан keystore)
...
# Проверить подпись релизного APK (v2 + v3 должны быть true):
~/Android/Sdk/build-tools/37.0.0/apksigner verify --verbose \
...
- **R8 / ProGuard — kotlinx.serialization**: `@Serializable` классы требуют keep-правил для companion-serializer'ов, иначе релизная сборка падает на первом `Json.decodeFromString`. Правила в `android/app/proguard-rules.pro`, взяты из официальной доки kotlinx.serialization. Не удалять.
- **R8 — Tink через `androidx.security.crypto`**: тянет compile-only аннотации (`com.google.errorprone.annotations.*`, `javax.annotation.*`). Без `-dontwarn` на них R8 ломает `minifyRelease` в ошибке «Missing class». Правила есть в `proguard-rules.pro`. Если появляются новые missing-class warning — смотреть сгенерированный `app/build/outputs/mapping/release/missing_rules.txt` и переносить оттуда.
- **R8 обфусцирует Compose-классы**: в `uiautomator dump` видно `v0.f0` вместо читаемых имён. Это нормально, не паниковать. Поиск элементов — по `text=` / `content-desc=`, не по `class=`.
...
- **Релиз v0.1.0 выпущен.** Следующий запланированный этап — будущая фаза VPS-туннеля (см. `docs/FUTURE-REMOTE-ACCESS.md`), но она отложена. До неё — bugfix и мелкие улучшения по запросу пользователя.

## Как выпустить новую версию (bump + release)

1. В `android/app/build.gradle.kts` поднять `versionCode` (целое, +1) и `versionName` (семвер, напр. `0.1.1`).
2. `./gradlew :app:assembleRelease` в `android/` — соберёт и подпишет. Проверить подпись: `apksigner verify --verbose` должен показать v2 + v3 = true.
3. Smoke-тест на реальном устройстве (`adb install -r app-release.apk`) — хотя бы открытие экрана подключения и pairing, чтобы убедиться что ничего не сломалось в сериализации/R8.
4. Коммит, push в `origin/main`.
5. `gh release create vX.Y.Z --target main --title "..." --notes "..." app/build/outputs/apk/release/app-release.apk` — публичный GitHub Release с APK.
